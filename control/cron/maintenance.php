...
Class cgMaintenance {
   ...
   function cleanOrders(){
	...
	/* Remove old/unused trades */
	function cleanTrades(){
	$ccDb = $this->connectDb();
       $stmt = $ccDb->prepare("SELECT basic_id from ccdev_trades WHERE timestamp <= DATE_ADD(NOW(), INTERVAL -24 HOUR)");
       $stmt->execute();
       $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
	...
	/* Remove old/unused cron orders */
	function cleanCron(){
	$ccDb = $this->connectDb();
       $stmt = $ccDb->prepare("SELECT basic_id from ccdev_cron_orders WHERE timestamp <= DATE_ADD(NOW(), INTERVAL -24 HOUR) AND status = :status");
       ...
       if(is_array($rows)) {
           foreach($rows as $row){
               $stmt2 = $ccDb->prepare("DELETE FROM ccdev_cron_orders WHERE basic_id = :id");
               $stmt2->bindValue(':id', $row['basic_id'], PDO::PARAM_INT);
               $stmt2->execute();
           }
       }
